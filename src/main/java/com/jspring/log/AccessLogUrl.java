...
public class AccessLogUrl {
	...
	public static AccessLogUrl newInstance(String fullUrl, String[] shortPaths) {
		int i = fullUrl.indexOf("://");
		if (i < 0) {
			int j = fullUrl.indexOf('/', i + 3);
			if (j < 0) {
				return newInstance("http", fullUrl, "/", shortPaths);
			}
			return newInstance("http", fullUrl.substring(0, j), fullUrl.substring(j), shortPaths);
		}
		int j = fullUrl.indexOf('/', i + 3);
		if (j < 0) {
			return newInstance(fullUrl.substring(0, i), fullUrl.substring(i + 3), "/", shortPaths);
		}
		return newInstance(fullUrl.substring(0, i), fullUrl.substring(i + 3, j), fullUrl.substring(j), shortPaths);
	}
	...
	public static AccessLogUrl newInstance(String protocal, String rootUrl, String pathAndQueryString,
		...
		if (null != shortPaths && shortPaths.length > 0) {
			for (String shortPath : shortPaths) {
				if (r.path.startsWith(shortPath)) {
					r.filename = r.path.substring(shortPath.length());
					r.path = shortPath;
					return r;
				}
			}
		}
		// 为了保持目录的简洁可聚合，查询目录中的动态部分（数字、简单符号），作为文件名处理，从目录中排除
		int activeIndex = i;
		for (int m = i - 1; m >= 0; m--) {
	...
	public static AccessLogUrl newInstanceWithWhiteList(String fullUrl, String[] shortPaths,
			String[] extentionWhiteList) {
		AccessLogUrl r = newInstance(fullUrl, shortPaths);
		int i = r.filename.lastIndexOf('.');
		if (i < 0) {
			return r;
		}
	...
	public static AccessLogUrl newInstanceWithWhiteList(String protocal, String rootUrl, String pathAndQueryString,
			String[] shortPaths, String[] extentionWhiteList) {
		AccessLogUrl r = newInstance(protocal, rootUrl, pathAndQueryString, shortPaths);
		int i = r.filename.lastIndexOf('.');
		if (i < 0) {
			return r;
		}
	...
	public static AccessLogUrl newInstanceWithBlackList(String fullUrl, String[] shortPaths,
	...
	public static AccessLogUrl newInstanceWithBlackList(String protocal, String rootUrl, String pathAndQueryString,
			String[] shortPaths, String[] extentionBlackList) {
		AccessLogUrl r = newInstance(protocal, rootUrl, pathAndQueryString, shortPaths);
		int i = r.filename.lastIndexOf('.');
		if (i < 0) {
			return r;
